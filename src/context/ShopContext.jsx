...
const ShopContextProvider = (props) => {
    ...
    const addToCart = async (itemId, delOption) => {
        if (!delOption) {
        ...
        if (cartData[itemId]) {
            if (cartData[itemId][delOption]) { // если выбрано то добавляем один продукт в корзину
                cartData[itemId][delOption] += 1
    ...
    const updateQuantity = async (itemId, delOption, quantity) => {

        let cartData = structuredClone(cartItems)

        cartData[itemId][delOption] = quantity // modified amount of products
    ...
    const value = {
      cartItems, addToCart, inCart, setInCart, delOption, setDelOption,  currency,
